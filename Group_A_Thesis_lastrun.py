...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Semester_7\\Thesis\\Thesis_exp_main\\thesis-main\\Group_A_Thesis_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "p_feedback" ---
    ...
    # --- Initialize components for Routine "self_other_instructions_2" ---
    text_10 = visual.TextStim(win=win, name='text_10',
        text="In this study, you will be shown two shapes (circle and diamond) with two labels (disgusting self and desirable self). You have to memorize this association. \n\n- Disgusting self is represented by a Diamond (Disgusting self-Diamond) and\n-Desirable self is represented by Circle (Desirable self-Circle) \n\nTake a few minutes to memorize this association. Let me know when you have memorized and ready to proceed to the experiment.\n\npress 'spacebar' to continue",
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    # --- Initialize components for Routine "self_other_intro_2" ---
    ...
    # --- Initialize components for Routine "self_other_starting_3" ---
    ...
    p_trials = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('A_block1.xlsx'), 
        seed=None, 
    )
    ...
    for thisP_trial in p_trials:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        p_text.setText(label)
        # create starting attributes for key_resp_p
        key_resp_p.keys = []
        key_resp_p.rt = []
        _key_resp_p_allKeys = []
        p_image.setImage(stimulus)
        ...
        if practice1.maxDurationReached:
            routineTimer.addTime(-practice1.maxDuration)
        elif practice1.forceEnded:
            routineTimer.reset()
        else:
            routineTimer.addTime(-1.600000)
        
        # --- Prepare to start Routine "p_feedback" ---
        # create an object to store info about Routine p_feedback
        p_feedback = data.Routine(
            name='p_feedback',
            components=[text_15],
        )
        p_feedback.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        ...
        # store start times for p_feedback
        p_feedback.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        p_feedback.tStart = globalClock.getTime(format='float')
        p_feedback.status = STARTED
        thisExp.addData('p_feedback.started', p_feedback.tStart)
        p_feedback.maxDuration = None
        # keep track of which components have finished
        p_feedbackComponents = p_feedback.components
        for thisComponent in p_feedback.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "p_feedback" ---
        # if trial has changed, end Routine now
        if isinstance(p_trials, data.TrialHandler2) and thisP_trial.thisN != p_trials.thisTrial.thisN:
            continueRoutine = False
        p_feedback.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 1.0:
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                p_feedback.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in p_feedback.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "p_feedback" ---
        for thisComponent in p_feedback.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for p_feedback
        p_feedback.tStop = globalClock.getTime(format='float')
        p_feedback.tStopRefresh = tThisFlipGlobal
        thisExp.addData('p_feedback.stopped', p_feedback.tStop)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if p_feedback.maxDurationReached:
            routineTimer.addTime(-p_feedback.maxDuration)
        elif p_feedback.forceEnded:
            routineTimer.reset()
        else:
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[fixation, fixation_3, text, key_resp, image],
        )
        ...
        while continueRoutine and routineTimer.getTime() < 1.6:
            ...
            if fixation_3.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
            ...
            if fixation_3.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > fixation_3.tStartRefresh + 0.3-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            if key_resp.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > key_resp.tStartRefresh + 1.1-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        if not key_resp.keys:  # No response was made
            ...
            feedback_message = 'Correct'
        elif key_resp.keys == 'z' and not is_match1:  # Correct not-match response
            feedback_message = 'Correct'
        else:  # Any other response is incorrect
            feedback_message = 'Incorrect'
    ...
    # --- Prepare to start Routine "self_other_instructions_2" ---
    # create an object to store info about Routine self_other_instructions_2
    self_other_instructions_2 = data.Routine(
        name='self_other_instructions_2',
        components=[text_10, key_resp_7],
    )
    self_other_instructions_2.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    key_resp_7.rt = []
    _key_resp_7_allKeys = []
    # store start times for self_other_instructions_2
    self_other_instructions_2.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    self_other_instructions_2.tStart = globalClock.getTime(format='float')
    self_other_instructions_2.status = STARTED
    thisExp.addData('self_other_instructions_2.started', self_other_instructions_2.tStart)
    self_other_instructions_2.maxDuration = None
    # keep track of which components have finished
    self_other_instructions_2Components = self_other_instructions_2.components
    for thisComponent in self_other_instructions_2.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "self_other_instructions_2" ---
    self_other_instructions_2.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            self_other_instructions_2.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in self_other_instructions_2.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "self_other_instructions_2" ---
    for thisComponent in self_other_instructions_2.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for self_other_instructions_2
    self_other_instructions_2.tStop = globalClock.getTime(format='float')
    self_other_instructions_2.tStopRefresh = tThisFlipGlobal
    thisExp.addData('self_other_instructions_2.stopped', self_other_instructions_2.tStop)
    # check responses
    ...
    thisExp.nextEntry()
    # the Routine "self_other_instructions_2" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # --- Prepare to start Routine "self_other_intro_2" ---
    # create an object to store info about Routine self_other_intro_2
    self_other_intro_2 = data.Routine(
        name='self_other_intro_2',
        components=[circle_image, diamond_image, Disgusting_self_lable, Desirable_self_lable, key_resp_8, text_11],
    )
    self_other_intro_2.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    key_resp_8.rt = []
    _key_resp_8_allKeys = []
    # store start times for self_other_intro_2
    self_other_intro_2.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    self_other_intro_2.tStart = globalClock.getTime(format='float')
    self_other_intro_2.status = STARTED
    thisExp.addData('self_other_intro_2.started', self_other_intro_2.tStart)
    self_other_intro_2.maxDuration = None
    # keep track of which components have finished
    self_other_intro_2Components = self_other_intro_2.components
    for thisComponent in self_other_intro_2.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "self_other_intro_2" ---
    self_other_intro_2.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            self_other_intro_2.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in self_other_intro_2.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "self_other_intro_2" ---
    for thisComponent in self_other_intro_2.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for self_other_intro_2
    self_other_intro_2.tStop = globalClock.getTime(format='float')
    self_other_intro_2.tStopRefresh = tThisFlipGlobal
    thisExp.addData('self_other_intro_2.stopped', self_other_intro_2.tStop)
    # check responses
    ...
    thisExp.nextEntry()
    # the Routine "self_other_intro_2" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # --- Prepare to start Routine "self_other_starting_3" ---
    # create an object to store info about Routine self_other_starting_3
    self_other_starting_3 = data.Routine(
        name='self_other_starting_3',
        components=[text_12, key_resp_9],
    )
    self_other_starting_3.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    ...
    key_resp_9.rt = []
    _key_resp_9_allKeys = []
    # store start times for self_other_starting_3
    self_other_starting_3.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
    self_other_starting_3.tStart = globalClock.getTime(format='float')
    self_other_starting_3.status = STARTED
    thisExp.addData('self_other_starting_3.started', self_other_starting_3.tStart)
    self_other_starting_3.maxDuration = None
    # keep track of which components have finished
    self_other_starting_3Components = self_other_starting_3.components
    for thisComponent in self_other_starting_3.components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "self_other_starting_3" ---
    self_other_starting_3.forceEnded = routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        tThisFlip = win.getFutureFlipTime(clock=routineTimer)
        ...
        # check if all components have finished
        if not continueRoutine:  # a component has requested a forced-end of Routine
            self_other_starting_3.forceEnded = routineForceEnded = True
            break
        continueRoutine = False  # will revert to True if at least one component still running
        for thisComponent in self_other_starting_3.components:
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
    ...
    # --- Ending Routine "self_other_starting_3" ---
    for thisComponent in self_other_starting_3.components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # store stop times for self_other_starting_3
    self_other_starting_3.tStop = globalClock.getTime(format='float')
    self_other_starting_3.tStopRefresh = tThisFlipGlobal
    thisExp.addData('self_other_starting_3.stopped', self_other_starting_3.tStop)
    # check responses
    ...
    thisExp.nextEntry()
    # the Routine "self_other_starting_3" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    p_trials_2 = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions('A_block2.xlsx'), 
        seed=None, 
    )
    ...
    for thisP_trial_2 in p_trials_2:
        ...
        # Check if they responded correctly, incorrectly, or missed
        is_match1 = int(is_match_2)  # is_match is now treated as a variable from the Excel file
    ...
    for thisTrial_2 in trials_2:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        text_8.setText(label_2)
        # create starting attributes for key_resp_6
        key_resp_6.keys = []
        key_resp_6.rt = []
        _key_resp_6_allKeys = []
        image_2.setImage(stimulus_2)
